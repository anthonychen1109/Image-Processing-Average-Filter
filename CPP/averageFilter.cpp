...
public:
    ...
    averageFilter(int n, int c, int min, int max){
        numRows = n;
        numCols = c;
        minVal = min;
        maxVal = max;
    ...
    void readImage(ifstream& inFile){
        ...
        averageFilter AF(numRows, numCols, minVal, maxVal);
    ...
    void getAverage(ofstream& outFile){
        for(int i=1; i<numRows+1; i++){
            for(int j=1; j<numCols+1; j++){
                int average = loadNeighbors(i,j);
                mirrorFramed[i][j] = average;
            }//for j
        }//for i
    }//getMedian
    
    void printAverage(ofstream& outFile){
    ...
    int loadNeighbors(int i, int j){
        ...
        int average = (a+b+c+d+x+f+g+h+k)/9;
...
int main(int argc, const char * argv[]) {
    ...
    AF.readImage(inFile); //read header
    AF.loadImage(inFile); //load image into mirrorFramedAry
    AF.mirrorFrame(outFile); //copy values to the frame
    AF.getAverage(outFile); //get median
    AF.printAverage(outFile);
    cout << "\n";
